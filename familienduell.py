...
def terminate_error(error_description):
    if messagebox.showerror("FAMILIENDUELL ERROR", error_description):
        sys.exit()
...
if filename == "":
    terminate_error("Keine Datei ausgewählt")

if filename[-4:] != ".csv":
    terminate_error("Falsches Dateiformat, muss es sein .csv")
...
with open(filename, "r+") as f:
    ...
    for j, line in enumerate(lines):
        ...
        if len(line) > 14:
            terminate_error(f"Jede Runde darf höchstens 7 Antworten haben, {line} hat {len(line)//2}")
        ...
        for i in range(0, len(line), 2):
            ...
            if len(round_answer) > 16:
                terminate_error(f"Antworten {round_answer} an der Linie {i-1} es ist zu lang")

            # Check if the  is valid
            points = line[i + 1]
            if not points.isdigit() or int(points) not in range(100):
                print(line[i])
                terminate_error(f"Punkte {points} an der Linie {j+1} sind vo gültig")
...
pygame.display.set_caption("Familienduell")
programIcon = pygame.image.load("familienduell.ico")
pygame.display.set_icon(programIcon)

# Create the second window
window1 = tkinter.Tk()
window1.title("Familienduell - Direktor")
window1.iconbitmap("familienduell.ico")
...
for i, round_answers in enumerate(game1.answers):
    ...
    round_button = tkinter.Button(round_tab, text="Beginne die Runde", command=lambda round=i: game1.round_init(round))
    round_button.grid(row=0, column=2, sticky="ew")

    l_start_button = tkinter.Button(round_tab, text="Zuerst links", command=lambda: game1.set_starting_team("L"))
    l_start_button.grid(row=1, column=1, sticky="ew")

    r_start_button = tkinter.Button(round_tab, text="Richtig zuerst", command=lambda: game1.set_starting_team("R"))
    r_start_button.grid(row=1, column=3, sticky="ew")

    l_strike_button = tkinter.Button(round_tab, text="Lews Fehler", command=lambda: game1.incorrect_answer("L"))
    l_strike_button.grid(row=2, column=1, sticky="ew")

    r_strike_button = tkinter.Button(round_tab, text="Rechtsfehler", command=lambda: game1.incorrect_answer("R"))
...
score_button = tkinter.Button(score_tab, text="Zeige Ergebnisse", command=game1.show_scores)
score_button.pack()
tabControl.add(score_tab, text="Satzzeichen")
...
for w in range(5):
    for t in range(2):
        ...
        final_button = tkinter.Button(final_tab, text="Sicht", command=lambda r=w, c=t, answer_txt=input_answer, point_txt=input_points: game1.show_final_answer(answer_txt, point_txt, r, c))
        input_answer.grid(row=w + 1, column=t * 3)
        input_points.grid(row=w + 1, column=t * 3 + 1)
        final_button.grid(row=w + 1, column=t * 3 + 2)

startfinal_button = tkinter.Button(final_tab, text="Starten Sie das Finale", command=game1.init_final_round)
startfinal_button.grid(row=0, column=2)

doubled_answer = tkinter.Button(final_tab, text="Doppelt", command=lambda: pygame.mixer.Sound.play(dubel_sound))
...
tabControl.add(final_tab, text="Finale")
...
while True:
    ...
    # Set the font
    myfont = pygame.font.Font("familienduell.ttf", font_height)
